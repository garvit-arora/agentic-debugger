...
export function ScoreBreakdown() {
  ...
  const totalPossible =
    score.base + (score.speedBonus > 0 ? score.speedBonus : 10);

  return (
    <div className="bg-[#050505] border border-white/[0.15] rounded-md overflow-hidden shadow-2xl flex flex-col h-full">
      <div className="px-6 py-4 border-b border-white/[0.15] bg-white/[0.04] flex items-center justify-between">
        <h3 className="text-[11px] font-black uppercase tracking-[0.2em] text-white/80 flex items-center gap-2">
          <Target size={14} className="text-emerald-500" />
          Performance Scorecard
        </h3>
        <div className="text-[10px] font-mono text-white/40">RIFT_S26_EVAL</div>
      </div>

      <div className="p-8 space-y-10 flex-1">
        {/* Horizontal Stacked Bar */}
        <div className="space-y-4">
          <div className="h-4 w-full bg-white/[0.08] rounded-full overflow-hidden flex border border-white/10">
            {segments.map((seg, i) => (
              <motion.div
                key={i}
                initial={{ width: 0 }}
                animate={{
                  width: `${Math.max(0, (seg.value / totalPossible) * 100)}%`,
                }}
        ...
        <div className="space-y-4">
          <div className="flex items-start gap-4 p-4 bg-white/[0.04] border border-white/[0.12] rounded-sm group hover:border-blue-500/30 transition-colors">
            ...
            <div className="flex-1">
              ...
              <p className="text-[10px] text-white/40 leading-relaxed font-medium">
                Standard baseline for autonomous repository healing.
              </p>
            </div>
          </div>

          <div className="flex items-start gap-4 p-4 bg-white/[0.04] border border-white/[0.12] rounded-sm group hover:border-emerald-500/30 transition-colors">
            ...
            <div className="flex-1">
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="text-[12px] font-mono font-bold text-emerald-400">
                  +{score.speedBonus} pts
                </span>
              </div>
              <p className="text-[10px] text-white/40 leading-relaxed font-medium">
                Earned for resolving issues in {summary.timeTakenSeconds}s
                (threshold &lt; 300s).
              </p>
            </div>
          </div>

          <div className="flex items-start gap-4 p-4 bg-white/[0.04] border border-white/[0.12] rounded-sm group hover:border-red-500/30 transition-colors">
            ...
            <div className="flex-1">
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="text-[12px] font-mono font-bold text-red-400">
                  -{score.commitPenalty} pts
                </span>
              </div>
              <p className="text-[10px] text-white/40 leading-relaxed font-medium">
                Penalty for {summary.commitsCount} commits (threshold 20).
              </p>
            </div>
          </div>
        </div>

        <div className="pt-6 border-t border-white/[0.15] flex items-center justify-between">
          <div className="flex flex-col">
            ...
            <motion.span
              ...
              {score.total}
              <span className="text-sm text-white/40 font-bold ml-2">
                / {totalPossible}
              </span>
            </motion.span>
          </div>
          <div className="h-16 w-16 rounded-full border border-white/10 flex items-center justify-center relative overflow-hidden group">
            ...
            <Zap
              size={24}
              className={cn(
                "text-emerald-500/20 group-hover:text-emerald-500 transition-colors",
                score.total > 100 && "text-emerald-500 animate-pulse",
              )}
            />
          </div>
        </div>
