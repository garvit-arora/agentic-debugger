...
export function RunSummaryCard() {
  ...
  const rows = [
    ...
    {
      label: "Iterations",
      value: summary.iterationsUsed + "/5",
      icon: RotateCcw,
    },
  ];

  return (
    <div className="bg-[#060606] border border-white/[0.12] rounded-lg overflow-hidden h-full flex flex-col">
      {/* Header row */}
      <div className="px-6 py-4 border-b border-white/[0.12] bg-white/[0.04] flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <span
            className={cn(
              "text-[9px] font-black px-3 py-1 rounded-md border uppercase tracking-widest",
      ...
      <div className="p-6 space-y-0 flex-1">
        {rows.map(({ label, value, icon: Icon }, i) => (
          <div
            ...
            <span className="text-[11px] font-medium text-white/70 max-w-[260px] truncate text-right font-mono">
              {value}
            </span>
          </div>
        ))}
