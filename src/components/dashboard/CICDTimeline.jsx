...
import { CheckCircle2, XCircle, Clock, ExternalLink, Play, Activity } from 'lucide-react'
   ...
   return (
      <div className="bg-[#050505] border border-white/[0.08] rounded-md overflow-hidden shadow-2xl flex flex-col h-full">
         <div className="px-6 py-4 border-b border-white/[0.08] bg-white/[0.02] flex items-center justify-between shrink-0">
            ...
            <div className="text-[9px] font-mono text-white/20 uppercase tracking-widest">REAL_TIME_STREAM</div>
         </div>
            ...
            <div className="relative">
               ...
               {timeline.length === 0 && !isRunning && (
                  <div className="py-20 flex flex-col items-center justify-center gap-4 opacity-10">
                     <Clock size={32} />
                     <span className="text-[10px] uppercase tracking-[0.4em] font-black">Awaiting Stream</span>
                  </div>
               )}
                  ...
                  {timeline.map((item, index) => (
                     ...
                     >
                        ...
                        <div className={cn(
                           ...
                           item.status === 'passed' ? "border-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.3)]" :
                              item.status === 'failed' ? "border-red-500 shadow-[0_0_8px_rgba(239,68,68,0.3)]" :
                                 "border-yellow-500 shadow-[0_0_8px_rgba(234,179,8,0.3)]"
                        )}>
                           {item.status === 'passed' ? <CheckCircle2 size={8} className="text-emerald-500" /> : <XCircle size={8} className="text-red-500" />}
                        </div>

                        <div className="flex flex-col gap-2">
                           <div className="flex items-center justify-between">
                              <div className="flex items-center gap-3">
                                 <span className="text-[12px] font-black text-white/80 uppercase tracking-widest">Iteration {item.iteration}</span>
                                 <span className={cn(
                                    "text-[9px] font-black px-2 py-0.5 rounded-sm border uppercase tracking-widest",
                                    item.status === 'passed' ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20" : "bg-red-500/10 text-red-500 border-red-500/20"
                                 )}>
                           ...
                           <p className="text-[11px] text-white/50 leading-relaxed font-medium">
                              {item.message || `Automated verification completed with status: ${item.status}`}
                           </p>
                           {item.duration && (
                              <div className="flex items-center gap-4 mt-1">
                                 ...
                                 <a href="#" className="flex items-center gap-1.5 text-[10px] text-blue-500/60 hover:text-blue-500 transition-colors font-bold uppercase tracking-widest">
                  ...
                  {isRunning && (
                     ...
                     >
                        ...
                        <div className="flex flex-col gap-1">
                           <span className="text-[12px] font-black text-blue-500 uppercase tracking-widest">Active Processing...</span>
                           <span className="text-[10px] text-white/20 font-medium">Synthesizing iteration {timeline.length + 1}</span>
                        </div>
                     </motion.div>
                  )}
               </div>
